...
| `invoice set-sending-status` | The Invoice API reports the sending status but exposes no status-only mutation.                                                                      | The official API sets a specific invoice to `sent` or `unsent` without delivering it and reports the resulting sending status.                                    |
