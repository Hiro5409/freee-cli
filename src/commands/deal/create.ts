...
import { initCommand, parseDate, parseNumber } from "../../helpers.ts";
...
$ freee deal-create --company-id 123 --date 2026-08-01 --type expense \\
  ...
  run: async (ctx) => {
    ...
    const body = {
      ...
      details: [
        {
          account_item_id: parseNumber(ctx.values["account-item-id"], "--account-item-id"),
          tax_code: parseNumber(ctx.values["tax-code"], "--tax-code"),
          amount: parseNumber(ctx.values.amount, "--amount"),
          description: ctx.values.description ?? "",
        },
      ],
      partner_id: ctx.values["partner-id"]
        ? parseNumber(ctx.values["partner-id"], "--partner-id")
        : undefined,
    };
    ...
    return formatValue(
      data,
      format,
      `${colors.green("Deal created:")} ${JSON.stringify(data, null, 2)}`,
    );
  },
});
