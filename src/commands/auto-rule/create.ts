...
/** freee encodes the match condition as an integer; the CLI takes the name. */
...
export const autoRuleCreateCommand = define({
  name: "auto-rule-create",
  description: "Create an auto-registration rule (act=auto_standard: registers a deal on match)",
  args: {
    ...writeArgs,
    description: {
      type: "string" as const,
      description: "Text the wallet txn description is matched against",
      required: true,
    },
    condition: {
      type: "string" as const,
  ...
  run: async (ctx) => {
    ...
    if (typeof minAmount === "number" && typeof maxAmount === "number" && minAmount > maxAmount) {
      throw new CliError(
        ...
        {
          why: "An empty amount range would never match any wallet txn.",
    ...
    return [
      colors.green(`Auto rule created: id=${data.id}`),
