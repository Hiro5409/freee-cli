...
import { invoiceListCommand } from "../../src/commands/invoice/list.ts";
import { saveCredentials } from "../../src/config/credentials.ts";
import { errorHints } from "../../src/errors.ts";
import { handleInvoicesIndex } from "../../src/types/freee-invoice/msw.gen.ts";
import { createMockInvoiceSummary, INVOICE_API_BASE_URL, MOCK_TOKEN } from "../fixtures.ts";
...
describe("invoice list command", () => {
  ...
  test("請求書APIの権限がなければ auth ヒント付きで説明する", async () => {
    server.use(
      handleInvoicesIndex(
        () =>
          HttpResponse.json(
    ...
    expect(cli(["--company-id", "123"], invoiceListCommand)).rejects.toMatchObject({
