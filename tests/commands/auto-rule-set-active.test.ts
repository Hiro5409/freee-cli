...
import {
  autoRuleDisableCommand,
  autoRuleEnableCommand,
} from "../../src/commands/auto-rule/set-active.ts";
import { saveCredentials } from "../../src/config/credentials.ts";
import { handleGetUserMatcher, handleUpdateUserMatcher } from "../../src/types/freee/msw.gen.ts";
import { createMockUserMatcher, MOCK_TOKEN } from "../fixtures.ts";

const testDir = join(tmpdir(), `freee-cli-auto-rule-set-active-test-${Date.now()}`);
...
describe("auto-rule disable command", () => {
  test("PUTs the full current state with only active flipped to false", async () => {
    currentActive = true;
    await cli(baseArgs, autoRuleDisableCommand);
  ...
  test("reports the new state", async () => {
    const result = await cli(baseArgs, autoRuleDisableCommand);
  ...
  test("dry-run fetches the rule but does not PUT, printing the payload", async () => {
    const result = await cli([...baseArgs, "--dry-run"], autoRuleDisableCommand);
  ...
  test("rejects a non-positive --id before calling the API", async () => {
    await expect(cli(["--company-id", "123", "--id", "0"], autoRuleDisableCommand)).rejects.toThrow(
      /positive integer/,
    );
...
describe("auto-rule enable command", () => {
  test("PUTs the full current state with only active flipped to true", async () => {
    currentActive = false;
    await cli(baseArgs, autoRuleEnableCommand);
  ...
  test("--format json returns the updated rule", async () => {
    currentActive = false;
    const result = await cli([...baseArgs, "--format", "json"], autoRuleEnableCommand);
