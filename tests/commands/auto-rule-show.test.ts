...
import { autoRuleShowCommand } from "../../src/commands/auto-rule/show.ts";
import { saveCredentials } from "../../src/config/credentials.ts";
import { handleGetUserMatcher } from "../../src/types/freee/msw.gen.ts";
import { createMockUserMatcher, MOCK_TOKEN } from "../fixtures.ts";

const testDir = join(tmpdir(), `freee-cli-auto-rule-show-test-${Date.now()}`);
...
describe("auto-rule show command", () => {
  test("fetches the rule by ID and outputs it as JSON", async () => {
    ...
    expect(rules[0].description).toBe("AMAZON");
  });

  test("rejects a non-positive --id before calling the API", async () => {
    await expect(cli(["--company-id", "123", "--id", "0"], autoRuleShowCommand)).rejects.toThrow(
      /positive integer/,
    );
