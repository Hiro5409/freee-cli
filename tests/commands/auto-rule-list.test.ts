...
import { autoRuleListCommand } from "../../src/commands/auto-rule/list.ts";
import { saveCredentials } from "../../src/config/credentials.ts";
import { handleGetUserMatchers } from "../../src/types/freee/msw.gen.ts";
import { createMockUserMatcher, MOCK_TOKEN } from "../fixtures.ts";

const testDir = join(tmpdir(), `freee-cli-auto-rule-list-test-${Date.now()}`);
...
describe("auto-rule list command", () => {
  test("outputs all rules as JSON, paginating with company_id/limit/offset", async () => {
    const result = await cli(baseArgs, autoRuleListCommand);
  ...
  test("rejects an unknown --active before calling the API", async () => {
    await expect(cli([...baseArgs, "--active", "on"], autoRuleListCommand)).rejects.toThrow(
      /active, inactive, all/,
    );
