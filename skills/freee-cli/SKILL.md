...
2. Run `freee docs list --format json`, then `freee docs show <name>` when a listed topic applies.
3. Run the selected command with `--help` before constructing unfamiliar arguments.
4. Use `--format json` for data consumed by an agent or another command.
5. For `setup` or `login`, ask the user to run the command in an interactive terminal.
6. Before a mutation, use read commands to verify the selected profile, company, and referenced IDs.
7. Run the complete mutation with `--dry-run --format json` and inspect the local request preview. A successful preview does not verify credentials, access, or referenced IDs with freee.
8. Present the preview and wait for explicit user approval. After approval, run the same command with only `--dry-run` removed.
